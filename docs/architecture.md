...
flowchart LR
    ...
    gateway[Vercel AI Gateway<br/>LLM + embeddings proxy]
    ...
    backend -->|generate grounded answer| gateway
    backend -->|stream answer + citations| browser

    corpus --> ingestion
    ingestion -->|create embeddings| gateway
...
- OpenAI SDK (configured to route through Vercel AI Gateway) for generation and embeddings
...
The frontend is responsible for user interaction, local UI state, and sending the authenticated user's request to the backend. It should never hold service-role credentials, run retrieval logic, call Vercel AI Gateway or OpenAI directly, or write privileged records to Supabase.
...
AI Co-Analyst uses hybrid retrieval:

1. Embed the user's query with the configured embedding model via Vercel AI Gateway.
...
- `AI_GATEWAY_URL`
- `AI_GATEWAY_API_KEY` (optional, if required by the gateway)
- `ALLOWED_ORIGINS`
...
- No direct OpenAI or Vercel AI Gateway calls from the browser.
